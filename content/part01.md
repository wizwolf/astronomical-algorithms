...
反三角函数(arcsine, arccosine, arctangent)并不是单值函数。例如，如果sinα=0.5，那么α=30°，150°，390°，等等。基于这个原因，在程序设计语言中，反三角函数的返回值范围，仅为0～360度的一半：反正弦和反正切的取值范围是－90～＋90度(也就是-π/2～+π/2弧度)，而反余弦的取值范围则是0～180度(也就是0～π弧度)。
