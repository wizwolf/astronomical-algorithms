...
赤经通常用时间的时分秒表示。计算赤经的三角函数，必需把赤经转换为度（然后，如需要再转换为弧度），记住1小时对应于15度。
  ...
  **例1.a** ——当α = 9h 14m 55s.8时，计算tan α。我们首先把α转化为以时为单位的十进制数：
  9h 14m 55s.8 ＝ 9＋14/60+55.8/3600=9.248833333时。
  然后再乘以15，我们得到α＝138°.73250。然后再乘以180/π=0.0174532925...得到α的弧度，
  然后得出 tan α=-0.877517。

## 正确的象限

当已知一个角的正弦、余弦或正切值时，可以通过反函数，反正弦（ASN或ASIN），反余弦(ACS或ACOS)或反正切(ATN或ATAN),求出角的值。但遗憾的是，在一些程序设计语言中，没有提供反正弦和反余弦函数。

反三角函数(arcsine, arccosine, arctangent)并不是单值函数。例如，如果sinα=0.5，那么α=30°，150°，390°，等等。基于这个原因，在程序设计语言中，反三角函数的返回值范围，仅为0～360度的一半：反正弦和反正切的返回值在－90～＋90度之间(也就是-π/2～+π/2弧度之间)，而反余弦的返回值在0～180度之间(也就是0～π弧度之间)。

例如，计算cos147°，结果是－0.8387，用反余弦函数计算－0.8387的结果是147°。但是计算cos213°的结果，也是－0.8387，对其用反余弦函数计算，结果则是147°。

因此，当使用SIN、COS和TAN的反函数时，消除这种歧义，有时是必要的。每个问题都必须单独检查。
